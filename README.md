...
**VK VWS Plugin Beta Tester**は、VK Blocks Proなどの VWS プラグインのベータ版を受け取り、テストするためのプラグインです。このプラグインを有効化すると、通常の安定版ではなく、ベータ版の更新を受け取ることができます。
...
安定版に緊急修正（セキュリティ修正等）をリリースする場合、ベータユーザーにも修正が届くよう、以下の手順で対応してください。

1. 通常どおり安定版の緊急修正をリリースする
2. 緊急修正を **同日中に** ベータブランチ（develop）にもマージする
3. マージ後、新しいベータ版をビルド・配信する

ベータテスターは `channel=beta` で更新を受け取るため、安定版のみリリースした場合はベータユーザーに修正が届きません。必ずベータブランチにもマージし、新しいベータ版を配信してください。
...
WP-CLIを使って手動でコンパイルします。

```bash
# POT ファイルを再生成
wp i18n make-pot --skip-js . languages/vk-vws-plugin-beta-tester.pot

# languages/vk-vws-plugin-beta-tester-ja.po を手動編集後、.l10n.php を生成
cd languages && wp i18n make-php .
```

`.l10n.php`はコンパイル済みファイルとしてgitにコミットします。
...
A: 現在はVK Blocks Proのみ対応しています。今後必要があれば、他のVWSプラグインにも対応予定です。
